...
│       ├── layout_pcb_smd_2d_dimension_x.png   # 2D-топология платы с габаритным размером по ширине (32.38 мм)
│       ├── layout_pcb_smd_2d_dimension_y.png   # 2D-топология платы с габаритным размером по высоте (24.12 мм)
│       ├── layout_pcb_smd_3d_view.png          # 3D-модель печатного узла усилителя в сборе
...
│       ├── schematic_smd_amplifier_2sk2974.png # Принципиальная схема СВЧ-тракта усилителя на SMD-компонентах
│       ├── schematic_smd_meander_bias_network.png # Схема четвертьволновых меандровых цепей питания с блокировкой C0G
...
│   ├── amplifier_2sk2974_1200mhz_smd_layout.emp # Проект с реальными SMD-компонентами, меандрами смещения и топологией платы
...
<p align="center">
  <img src="docs/images/schematic_smd_amplifier_2sk2974.png" alt="Схема СВЧ-тракта усилителя на SMD-компонентах" width="900"/>
  <br>
  <em>Рисунок 3 — Принципиальная схема СВЧ-тракта усилителя на SMD-компонентах в Cadence AWR Microwave Office</em>
</p>

<p align="center">
  <img src="docs/images/schematic_smd_meander_bias_network.png" alt="Четвертьволновые меандровые шлейфы питания" width="900"/>
  <br>
  <em>Рисунок 4 — Четвертьволновые меандровые шлейфы цепей подачи смещения с блокировочными конденсаторами C0G 1206</em>
</p>
...
- **Входная параллельная емкость**: параллельное включение конденсаторов ATC серии 100A — `SUBCKT ID=S9 NET="100A1R5B"` ($1.5\text{ пФ}$) и `SUBCKT ID=S8 NET="100A120F"` ($12\text{ пФ}$) (суммарная емкость $13.5\text{ пФ}$).
- **Выходная согласующая индуктивность**: прецизионная тонкопленочная СВЧ-индуктивность 0402 AVX Accu-L — `SUBCKT ID=S4 NET="L0402SEr56"` ($0.56\text{ нГн}$).
- **Выходная параллельная емкость**: конденсаторы ATC серии 100A — `SUBCKT ID=S11 NET="100A0R5"` ($0.5\text{ пФ}$) и `SUBCKT ID=S5 NET="100A100F"` ($10\text{ пФ}$) (суммарная емкость $10.5\text{ пФ}$).
- **Цепи смещения (RF Choke)**: четвертьволновые линии, свернутые в компактные меандры (`MBENDA` $90^\circ$ и отрезки `MLIN` шириной $W = 1.576\text{ мм}$ под $Z_0 = 50\ \Omega$), нагруженные на ВЧ-блокировочные конденсаторы `SUBCKT ID=S13, S7 NET="c0g1206_2n2_100V_shunt"` ($2.2\text{ нФ}$, $100\text{ В}$, SMD 1206, диэлектрик C0G/NP0) на землю.
- **Транзистор**: подсхема `SUBCKT ID=S10 NET="2SK2974"`, подключенная к файлу `simulation/2sk2974.s2p`.
- **Микрополосковые подводящие линии и разъемы**: 50-омные переходы `SUBCKT NET="SMA_Measured_Thru"`, линии `MLIN ID=TL5, TL6` ($W = 2\text{ мм}$, $L = 1\text{ мм}$) и тройники `MTEE` ($W = 5\text{ мм}$).

### 4. Топология печатной платы (PCB Layout)

<p align="center">
  <img src="docs/images/layout_pcb_smd_2d_dimension_x.png" alt="Топология печатной платы с габаритом по ширине" width="850"/>
  <br>
  <em>Рисунок 5 — Двумерная топология платы в AWR Microwave Office: измерение габаритной ширины ($32.38\text{ мм}$)</em>
</p>

<p align="center">
  <img src="docs/images/layout_pcb_smd_2d_dimension_y.png" alt="Топология печатной платы с габаритом по высоте" width="850"/>
  <br>
  <em>Рисунок 6 — Двумерная топология платы в AWR Microwave Office: измерение габаритной высоты ($24.12\text{ мм}$)</em>
</p>

<p align="center">
  <img src="docs/images/layout_pcb_smd_3d_view.png" alt="3D-модель печатного узла усилителя 2SK2974" width="850"/>
  <br>
  <em>Рисунок 7 — Трехмерная модель сборки печатного узла усилителя (3D Layout View)</em>
</p>

- **Габариты платы**: сверхкомпактный печатный узел с внешними размерами **$32.38 \times 24.12\text{ мм}$** (площадь подложки $\approx 7.8\text{ см}^2$).
- **Меандровые дроссели питания**: высокочастотные четвертьволновые шлейфы питания затвора и стока свернуты в меандры с плавными сглаженными изгибами $90^\circ$, что позволило уменьшить вертикальный габарит платы более чем на 50%.
- **Блокировка по питанию**: контактные площадки под SMD-конденсаторы типоразмера 1206 емкостью $2.2\text{ нФ}$ со сквозным заземлением через группы металлизированных переходных отверстий.
- **Экранирование и теплоотвод**: частокол заземляющих переходных отверстий (via fence) вдоль сигнального тракта и цепей питания, копланарные земляные полигоны, а также центральный вырез с металлизированной посадочной площадкой под силовой фланец транзистора 2SK2974 ([`cad/2sk2974_awr_footprint.dxf`](cad/2sk2974_awr_footprint.dxf)) для монтажа на теплоотводящий радиатор.
- **СВЧ-порты**: концевые контактные площадки для пайки торцевых разъемов SMA End-Launch с непрерывным переходом на 50-омные микрополосковые линии.
...
<p align="center">
  ...
  <br>
  <em>Рисунок 8 — Диаграммы Вольперта-Смита идеальной схемы: S11 (слева) и S22 (справа)</em>
</p>
...
<p align="center">
  ...
  <br>
  <em>Рисунок 9 — Частотные зависимости |S11| (слева) и коэффициента усиления |S21| (справа) идеальной схемы</em>
</p>

<p align="center">
  <img src="docs/images/plot_vswr_frequency_response.png" alt="Частотная зависимость КСВН" width="700"/>
  <br>
  <em>Рисунок 10 — Частотная зависимость КСВН по входу идеальной схемы</em>
</p>
...
<p align="center">
  ...
  <br>
  <em>Рисунок 11 — Диаграмма Вольперта-Смита для микрополосковой схемы: S11 (треугольник) и S22 (квадрат) на частоте 1.2 ГГц</em>
</p>
...
<p align="center">
  ...
  <br>
  <em>Рисунок 12 — Параметры микрополоскового усилителя на частоте 1.2 ГГц: КСВН портов 1 и 2 (слева) и коэффициент усиления |S21| (справа)</em>
</p>
...
<p align="center">
  ...
  <br>
  <em>Рисунок 13 — Диаграмма Вольперта-Смита схемы на реальных SMD-компонентах: S(1,1) и S(2,2) в полосе 1.20–1.21 ГГц</em>
</p>

<p align="center">
  <img src="docs/images/plot_smd_vswr_frequency_response.png" alt="КСВН схемы на SMD-компонентах" width="750"/>
  <br>
  <em>Рисунок 14 — Частотная зависимость КСВН усилителя на реальных SMD-компонентах в диапазоне 0.5–1.5 ГГц</em>
</p>

<p align="center">
  <img src="docs/images/plot_smd_s21_gain.png" alt="Усиление схемы на SMD-компонентах" width="750"/>
  <br>
  <em>Рисунок 15 — Частотная зависимость коэффициента передачи |S21| (усиления) в диапазоне 0.5–1.5 ГГц</em>
</p>
